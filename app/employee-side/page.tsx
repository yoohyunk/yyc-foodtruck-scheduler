...
export default function EmployeeDashboard() {
    ...
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        try {
          ...
          // Debug logs to help diagnose email matching issues
          console.log(
            "All employee emails:",
            employees.map((emp) => emp.user_email)
          );
          ...
          if (employee) {
            ...
            console.log("No employee profile found, continuing without it");
          }
        } catch {
          console.log("Error fetching employee profile, continuing without it");
        }
        ...
        const scheduleItems: ScheduleItem[] = await Promise.all(
          assignments.map(
            async (assignment: {
              ...
              try {
                // Get event details
                const event = await eventsApi.getEventById(assignment.event_id);

                return {
                  truck_name: event?.title || "Event",
                  ...
                  location: event?.addresses?.street || "Event Location",
                };
              } catch (error) {
        ...
        if (employee?.employee_id) {
          try {
            ...
            console.log("Error fetching time-off by employee ID");
          }
        }

        // Method 2: Try by user email
        if (timeOffRequests.length === 0 && user.email) {
          try {
            timeOffRequests =
              await timeOffRequestsApi.getTimeOffRequestsByEmployeeId(
                user.email
            ...
            console.log("Error fetching time-off by email");
          }
        }
  ...
  const hoursThisWeek = upcomingSchedule.reduce((total, shift) => {
    ...
    if (shiftDate >= startOfWeek && shiftDate <= endOfWeek) {
      ...
      const [startHour, startMinute] = shift.shift_start.split(":");
      const [endHour, endMinute] = shift.shift_end.split(":");
  ...
  return (
    <div className="bg-gray-50 min-h-screen w-full p-8">
      <div className="w-full flex flex-col gap-12">
        {/* Setup Notice - Only show if no employee profile */}
        {(employeeData === null || !employeeData.first_name) && !isLoading && (
          <div className="bg-blue-50 border border-blue-200 text-blue-800 px-4 py-3 rounded-lg">
            <div className="flex items-center justify-between">
              <div>
                <strong>Welcome!</strong> Set up your employee profile to see
                your schedule and time-off requests.
              </div>
              <a
                href="/set-up-employee-info"
