...
import taxCalculations from "../scheduledPayroll/taxCalculations.json";
...
export default function ClockedPayrollReport(): ReactElement {
  ...
  // Fetch clocked payroll data for the current pay period
  const fetchPayrollData = useCallback(async () => {
    ...
    try {
      ...
      // Bulk fetch all clocked sessions for all employees within the pay period
      const { data: allClockedSessions, error: clockedError } = await supabase
        ...
        .gte("clock_in_at", `${payPeriod.start}T00:00:00`)
        .lte("clock_in_at", `${payPeriod.end}T23:59:59`)
        .not("clock_out_at", "is", null); // Only completed sessions

      if (clockedError) {
        console.error("Error fetching clocked sessions:", clockedError);
        throw new Error("Failed to fetch clocked sessions");
      }
      ...
      for (const employee of sortedEmployees) {
        ...
        // Filter clocked sessions for this employee
        const employeeClockedSessions = (allClockedSessions || []).filter(
          (session) => session.assignments?.employee_id === employee.employee_id
        );
        ...
        // Map clocked sessions to assignments format for consistency
        const periodAssignmentsEnriched = employeeClockedSessions.map(
          (session) => {
            const clockInTime = new Date(session.clock_in_at!);
            const clockOutTime = new Date(session.clock_out_at!);
            const hours =
            ...
            const eventTitle =
              session.assignments?.events?.title || "Unknown Event";
            shiftDetails.push({
              type: "Server (Clocked)",
              title: eventTitle,
              start: clockInTime,
            ...
            return {
              id: session.id,
              start_date: session.clock_in_at!,
              end_date: session.clock_out_at!,
              events: {
                title: eventTitle,
              },
              isHoliday: isStatHoliday(clockInTime),
            };
          }
        );

        // Calculate gross pay with holiday multiplier
        let grossPay = 0;
        for (const session of employeeClockedSessions) {
          const clockIn = new Date(session.clock_in_at!);
          const clockOut = new Date(session.clock_out_at!);
          const hours =
            (clockOut.getTime() - clockIn.getTime()) / (1000 * 60 * 60);

          if (isStatHoliday(clockIn)) {
            grossPay += hours * currentWage * 1.5; // Holiday pay
          } else {
            grossPay += hours * currentWage;
          }
        }
        ...
        payrollDataArray.push({
          ...
          currentWage: wage,
          totalHours,
          totalEvents: employeeClockedSessions.length,
          grossPay,
          netPay,
          taxDeductions,
      ...
      console.error("Error fetching clocked payroll data:", err);
      setError("Failed to load clocked payroll data. Please try again.");
    } finally {
  ...
  if (!isAdmin && !user) {
    return (
      <div className="clocked-payroll-report">
        <h2 className="text-2xl text-primary-dark mb-4">
          Clocked Payroll Report
        </h2>
        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
          <p className="text-red-800">
            Please log in to view your clocked payroll information.
          </p>
        </div>
      </div>
    );
    ...
    <div className="clocked-payroll-report">
      <div className="mb-6">
        <div className="flex items-center mb-4">
          ...
          <TutorialHighlight
            isHighlighted={shouldHighlight("clocked-payroll-report")}
          >
        ...
        <p className="text-gray-600">
          {isAdmin
            ? "View employee hours and wages based on actual clocked time for the selected pay period."
            : "View your hours and wages based on actual clocked time for the selected pay period."}
        </p>
      </div>
      ...
      {!isLoading && payrollData.length > 0 && (
        <div className="mb-6 p-4 bg-green-50 border border-green-200 rounded-lg">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 text-sm">
            ...
            <div>
              <span className="text-green-600">Total Sessions:</span>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {payrollData.length > 0 ? (
                    ...
                    <tr>
                      ...
                      >
                        No clocked payroll data found for the selected pay
                        period.
                      </td>
                    </tr>
                  )}
          ...
          <div className="md:hidden space-y-4">
            {payrollData.length > 0 ? (
              ...
              <div className="text-center py-8 text-gray-500">
                No clocked payroll data found for the selected pay period.
              </div>
            )}
          </div>
        </>
